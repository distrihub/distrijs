...
import { DistriAnyTool, DistriUiToolLike, ToolCallStatus, RenderingMode, ChatSessionSettings, StreamingIndicator } from './types';
...
/**
 * Context handed to a `renderTool` callback so the framework layer can build
 * whatever "component" payload makes sense for it (a React element, an
 * Angular template descriptor, or nothing at all). @distri/state never
 * constructs UI itself — this is the one seam where framework code plugs in.
 */
export interface ToolRenderContext {
  kind: 'ui' | 'confirm' | 'error';
  tool?: DistriAnyTool;
  toolCall?: ToolCall;
...
export type RenderToolFn = (ctx: ToolRenderContext) => unknown;
...
export interface ToolCallState {
  ...
  endTime?: number;
  /**
   * Framework-owned render payload produced by the `renderTool` callback
   * passed to `createChatStore`. Opaque here by design — e.g. a React
   * element in @distri/react, or left `undefined` when no callback is
   * supplied (function tools still auto-execute/confirm via data either
   * way; only UI-type tool calls lose their visual, not their behavior).
   */
  component?: unknown;
...
export interface CreateChatStoreOptions {
  /**
   * Builds the framework-specific `component` payload for a tool call.
   * Called for UI-type tools, confirm-required function tools, and error
   * fallbacks. Omit it (e.g. in a headless/no-UI consumer) and tool calls
   * simply have no `component` — auto-executing function tools are
   * unaffected either way, since they never render one.
   */
  renderTool?: RenderToolFn;
}

/**
 * Factory for a fresh, isolated chat-state store. State (messages, streaming
 * flags, tasks, todos, browser session, …) lives for exactly as long as the
 * owning chat instance — construct a new store per conversation instead of
 * reusing/clearing a shared one.
 */
export function createChatStore(options: CreateChatStoreOptions = {}): ChatStore {
  const { renderTool } = options;

  // Per-instance dedup guard: prevents duplicate completion posts when a
  // framework's dev-mode double-invokes handlers (e.g. React StrictMode).
  // Scoped to this store so two chats can't suppress each other's tool
  // completions.
  ...
  processMessage: (message: DistriChatMessage, isFromStream: boolean = false) => {
    ...
    if (isDistriEvent(message)) {
      ...
      switch (message.type) {
        ...
        case 'agent_handover': {
          const handoverEvent = event as AgentHandoverEvent;
    ...
    try {
      ...
      try {
        if (distriTool?.type === 'ui') {
          const uiTool = distriTool as DistriUiToolLike;
          ...
          if (fnTool.autoExecute) {
            ...
            // The framework's `renderTool` handles its own pending → running
        ...
        const message = `Error loading tool: ${componentError instanceof Error ? componentError.message : 'Unknown component error'}`;
        component = renderTool?.({ kind: 'error', message });
      ...
      get().updateToolCallStatus(toolCall.tool_call_id, {
        ...
        component: renderTool?.({
          kind: 'error',
          message: `Critical tool error: ${error instanceof Error ? error.message : 'Unknown error'}`,
        }),
      });
    }
  },
...
function validateToolCallInput(toolCall: ToolCall): string | null {
