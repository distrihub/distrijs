...
export interface EmbeddableChatProps {
  ...
  HeaderComponent?: React.ComponentType<{ agent: Agent; agentId: string }>;
...
export const EmbeddableChat: React.FC<EmbeddableChatProps> = ({
  ...
  const { agent: internalAgent, loading: agentLoading } = useAgent({ 
    agentId: providedAgent ? undefined : agentId 
  });
  ...
  const DefaultHeader = () => (
    <div className={`flex-shrink-0 px-4 py-3 border-b ${borderClass} ${headerBgClass}`}>
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className={`font-semibold ${textClass}`}>{agent?.name || 'Assistant'}</h2>
    ...
    return (
      <div className={`flex items-center justify-center ${bgClass} ${className}`} style={{ height }}>
        <div className="text-center">
          {ErrorComponent ? <ErrorComponent error={noAgentError} /> : (
            <>
  ...
  return (
    <div className={`flex flex-col ${bgClass} ${textClass} ${className}`} style={{ height }}>
      ...
      {showHeader && (HeaderComponent ? 
        <HeaderComponent agent={agent} agentId={agentId} /> : 
        <DefaultHeader />
      )}
