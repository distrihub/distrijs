...
// ---------------------------------------------------------------------------
// Internal hook — uses DistriHomeClient for detailed thread data
// ---------------------------------------------------------------------------
...
function useDetailedThreads(initialParams: DetailedThreadListParams): ThreadsResultState {
  ...
  return { threads, total, page, pageSize, loading, error, params, setParams, nextPage, prevPage, setPageSize };
}

// ---------------------------------------------------------------------------
// AgentSearchDropdown (internal)
// ---------------------------------------------------------------------------
...
function AgentSearchDropdown({
  ...
  const displayName = selectedAgent
    ...
    : allowAll ? 'All agents' : 'Select agent';
            ...
            <div className="max-h-56 overflow-y-auto py-1">
              {allowAll && (
                <button
                  ...
                  className={`flex w-full items-center px-3 py-2 text-left text-sm transition hover:bg-muted/40 ${!selectedAgentId ? 'bg-muted/40 font-medium text-foreground' : 'text-muted-foreground'}`}
                >
                  All agents
                </button>
              )}
              {filteredAgents.length === 0 ? (
                ...
                filteredAgents.map((agent) => (
                  <button
                    ...
                    className={`flex w-full items-center justify-between px-3 py-2 text-left text-sm transition hover:bg-muted/40 ${agent.agent_id === selectedAgentId ? 'bg-muted/40 font-medium text-foreground' : 'text-foreground'}`}
                  >
                    <span className="truncate">{agent.agent_name}</span>
                    {agent.thread_count > 0 && (
                      <span className="ml-2 shrink-0 text-xs text-muted-foreground">{agent.thread_count} threads</span>
                    )}
                  </button>
                ))
              )}
...
function getTimeFilterDate(filter: QuickTimeFilter): string | undefined {
  ...
  const now = new Date();
  switch (filter) {
    case '5m': return new Date(now.getTime() - 5 * 60 * 1000).toISOString();
...
export interface ThreadListSlots {
  /** Per-row action buttons (e.g. custom cloud trace button) */
  rowActions?: (threadId: string) => ReactNode;
}

export interface ThreadListProps {
  slots?: ThreadListSlots;
  onAction?: (
    a:
      | { type: 'thread.opened'; id: string; agentId?: string }
      | { type: 'thread.traces'; id: string },
  ) => void;
  className?: string;
...
/**
 * ThreadList — Tier-2 block rendering the detailed threads table with search,
 * quick-time-filter, agent filter dialog, and pagination.
 * Does NOT include full-page header chrome (Task 11 handles that).
 */
export function ThreadList({
  ...
  const threadRowActions = slots?.rowActions ?? home.slots?.threadRowActions;
  ...
  const handleQuickTimeFilter = useCallback(
    (filter: QuickTimeFilter) => {
      if (quickTimeFilter === filter) {
        ...
        setParams({ ...params, from_date: getTimeFilterDate(filter), to_date: undefined, offset: 0 });
      }
    },
    ...
    <div className={`flex flex-col gap-6 ${className ?? ''}`}>
      ...
      {error && (
        <div className="flex items-start gap-3 rounded-lg border border-amber-300/50 bg-amber-100/60 px-4 py-3 text-xs text-amber-900 dark:border-amber-400/20 dark:bg-amber-400/10 dark:text-amber-100">
          ...
          <div>
            <p className="font-semibold">Could not load threads right now.</p>
            <p className="mt-1 text-amber-800/90 dark:text-amber-100/90">Try refreshing soon.</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {(['5m', '1h', '24h', '7d'] as const).map((filter) => (
          <button
            ...
            className={`rounded-md px-3 py-1.5 text-xs font-medium transition ${quickTimeFilter === filter ? 'bg-primary text-primary-foreground' : 'border border-border/70 bg-card text-muted-foreground hover:text-foreground'}`}
          >
        ...
        <div className="divide-y divide-border/60">
          {loading ? (
            ...
            <div className="px-6 py-4 text-sm text-muted-foreground">
              Could not load threads. Please try again shortly.
            </div>
          ) : threads.length === 0 ? (
            ...
            threads.map((thread) => (
              <div
                key={thread.id}
                onClick={() => onAction?.({ type: 'thread.opened', id: thread.id, agentId: thread.agent_id })}
              ...
              >
                <div className="min-w-0 flex-1 overflow-hidden">
                  <div className="flex items-baseline gap-3">
                    ...
                    {(thread.total_tokens ?? 0) > 0 && (
                      <span
                        ...
                        title={`Total: ${formatTokens(thread.total_tokens ?? 0)}`}
                      >
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); thread.agent_id && handleAgentClick(thread.agent_id); }}
                    ...
                    {thread.external_id && (
                      <button
                        type="button"
                        onClick={(e) => { e.stopPropagation(); handleExternalIdClick(thread.external_id); }}
                ...
                <div className="flex items-center gap-1">
                  {/* Slot-injected row actions */}
                  {threadRowActions ? (
                    threadRowActions(thread.id)
                  ) : (
                    <>
                      {/* Traces shortcut */}
                      <button
                        type="button"
                        onClick={(e) => { e.stopPropagation(); onAction?.({ type: 'thread.traces', id: thread.id }); }}
                      ...
                      {thread.agent_id && (
                        <button
                          type="button"
                          onClick={(e) => { e.stopPropagation(); onAction?.({ type: 'thread.opened', id: '', agentId: thread.agent_id }); }}
                          className="flex items-center gap-2 rounded-full p-2 text-muted-foreground opacity-0 transition hover:text-primary group-hover:opacity-100"
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      onAction?.({ type: 'thread.opened', id: thread.id, agentId: thread.agent_id });
                    }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-md rounded-xl border border-border bg-card p-6 shadow-xl">
            ...
            <div className="space-y-4">
              ...
              {[
                { label: 'External ID', value: dialogExternalId, onChange: setDialogExternalId, placeholder: 'Filter by external ID...' },
                { label: 'User ID', value: dialogUserId, onChange: setDialogUserId, placeholder: 'Filter by user ID...' },
                { label: 'Channel ID', value: dialogChannelId, onChange: setDialogChannelId, placeholder: 'Filter by channel ID...' },
                { label: 'Bot ID', value: dialogBotId, onChange: setDialogBotId, placeholder: 'Filter by bot ID...' },
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  { label: 'From Date', value: dialogFromDate, onChange: setDialogFromDate },
                  { label: 'To Date', value: dialogToDate, onChange: setDialogToDate },
