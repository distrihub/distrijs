  ...
  return (
    <section className="p-10 space-y-8">
      <header className="flex items-center justify-between">
        <div>
          <p className="text-sm uppercase tracking-[0.35em] text-slate-600">Launchpad</p>
          <h1 className="text-4xl font-semibold text-white mt-1">Campaigns</h1>
          <p className="text-slate-400 mt-2">Bundle bots into orchestrated pushes.</p>
        </div>
        <button
          type="button"
          onClick={() => navigate('/campaigns/new')}
          className="h-11 px-5 rounded-xl bg-indigo-500 text-white text-sm font-semibold"
        >
          Add Campaign
        </button>
      </header>
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          campaigns.map((campaign) => (
            <article key={campaign.id} className="bg-slate-900/40 border border-slate-800 rounded-2xl p-5 space-y-3">
